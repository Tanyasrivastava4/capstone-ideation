...
REQUIRED_COLUMNS = ["patient_id", "report_text"]
...
def validate_input_file(file_path: str) -> bool:
    ...
    if missing_cols:
        print(f"❌ Missing required columns: {missing_cols}")  # ✅ FIXED: was printf
        return False
    
    # 4️⃣ Basic data sanity checks
    if df.empty:
        print("❌ Error: The dataset is empty")
        return False
    
    if df["report_text"].isnull().any():
        print("⚠️ Warning: Some reports have missing text entries")
